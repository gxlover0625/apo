...
def main(_):
  ...
  if dataset_name == "mmlu":
    ...
    root_data_folder_path = os.path.join(
      ROOT_DATA_FOLDER_PATH, "Human_Group"
    )
  else:
    assert dataset_name == "gsm8k"
  ...
  for t in tasks_all:
    if dataset_name == "mmlu":
      ...
      processed_dfs = {}
      for split in splits:
        df_bbh = pd.read_csv(os.path.join(root_data_folder_path, "bbh_logical", f"{split}.csv"))
        df_bbeh = pd.read_csv(os.path.join(root_data_folder_path, "bbeh_boardgame", f"{split}.csv"))
        min_len = min(len(df_bbh), len(df_bbeh))
        combined = pd.concat([df_bbh[:min_len], df_bbeh[:min_len]], ignore_index=True)
        processed_dfs[split] = combined.sample(frac=1, random_state=42).reset_index(drop=True)
      ...
      processed_dfs = {}
      for split in splits:
        df_1 = pd.read_json(os.path.join(root_data_folder_path, "gaokao_geography", f"gaokao_geography_{split}.jsonl"), lines=True)
        df_2 = pd.read_json(os.path.join(root_data_folder_path, "gaokao_history", f"gaokao_history_{split}.jsonl"), lines=True)
        min_len = min(len(df_1), len(df_2))
        combined = pd.concat([df_1[:min_len], df_2[:min_len]], ignore_index=True)
        processed_dfs[split] = combined.sample(frac=1, random_state=42).reset_index(drop=True)
  ...
  elif dataset_name in ["geo_group", "logical_group"]:
    indices = np.arange(num_examples)
