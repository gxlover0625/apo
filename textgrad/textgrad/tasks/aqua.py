...
class AQUA(Dataset):
    def __init__(self, root:str=None, split:str="train"):
        ...
        train_path = Path(root) / "AGIEval_AQUA" / "aqua_train.jsonl"
        val_path = Path(root) / "AGIEval_AQUA" / "aqua_validation.jsonl"
        test_path = Path(root) / "AGIEval_AQUA" / "aqua_test.jsonl"
        if split == "train":
