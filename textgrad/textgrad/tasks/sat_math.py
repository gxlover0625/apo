...
class SATMath(Dataset):
    def __init__(self, root:str=None, split:str="train"):
        assert split in ["train", "val", "test"]
        train_path = Path(root) / "AGIEval_SAT" / "sat_train.jsonl"
        val_path = Path(root) / "AGIEval_SAT" / "sat_validation.jsonl"
        test_path = Path(root) / "AGIEval_SAT" / "sat_test.jsonl"
        if split == "train":
