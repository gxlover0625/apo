...
def load_task(task_name: str, evaluation_api: EngineLM, *args, **kwargs) -> Tuple[Dataset, Dataset, Callable]:
    ...
    if "object_counting" in task_name:
    ...
    elif task_name == "agieval_gaokao_math":
        from .gaokao_math import GaoKaoMath
        from .mygpqa import gpqa_eval_fn
        from textgrad.autograd.string_based_ops import StringBasedFunction
        train_set = GaoKaoMath(root=kwargs.get("data_dir"), split="train")
        val_set = GaoKaoMath(root=kwargs.get("data_dir"), split="val")
        test_set = GaoKaoMath(root=kwargs.get("data_dir"), split="test")
        fn_purpose = "The runtime of string-based function that checks if the prediction is correct."
        eval_fn = StringBasedFunction(gpqa_eval_fn, function_purpose=fn_purpose)
        return train_set, val_set, test_set, eval_fn

    elif task_name == "agieval_gaokao_history":
        from .gaokao_history import GaoKaoHistory
        from .mygpqa import gpqa_eval_fn
        from textgrad.autograd.string_based_ops import StringBasedFunction
        train_set = GaoKaoHistory(root=kwargs.get("data_dir"), split="train")
        val_set = GaoKaoHistory(root=kwargs.get("data_dir"), split="val")
        test_set = GaoKaoHistory(root=kwargs.get("data_dir"), split="test")
        fn_purpose = "The runtime of string-based function that checks if the prediction is correct."
        eval_fn = StringBasedFunction(gpqa_eval_fn, function_purpose=fn_purpose)
        return train_set, val_set, test_set, eval_fn
    
    elif task_name == "agieval_gaokao_chinese":
        from .gaokao_chinese import GaoKaoChinese
        from .mygpqa import gpqa_eval_fn
        from textgrad.autograd.string_based_ops import StringBasedFunction
        train_set = GaoKaoChinese(root=kwargs.get("data_dir"), split="train")
        val_set = GaoKaoChinese(root=kwargs.get("data_dir"), split="val")
        test_set = GaoKaoChinese(root=kwargs.get("data_dir"), split="test")
        fn_purpose = "The runtime of string-based function that checks if the prediction is correct."
        eval_fn = StringBasedFunction(gpqa_eval_fn, function_purpose=fn_purpose)
        return train_set, val_set, test_set, eval_fn
    
    elif task_name == "agieval_gaokao_geography":
        from .gaokao_geography import GaoKaoGeography
        from .mygpqa import gpqa_eval_fn
        from textgrad.autograd.string_based_ops import StringBasedFunction
        train_set = GaoKaoGeography(root=kwargs.get("data_dir"), split="train")
        val_set = GaoKaoGeography(root=kwargs.get("data_dir"), split="val")
        test_set = GaoKaoGeography(root=kwargs.get("data_dir"), split="test")
        fn_purpose = "The runtime of string-based function that checks if the prediction is correct."
        eval_fn = StringBasedFunction(gpqa_eval_fn, function_purpose=fn_purpose)
        return train_set, val_set, test_set, eval_fn
    
    elif task_name == "agieval_sat":
        from .sat_math import SATMath
        from .mygpqa import gpqa_eval_fn
        from textgrad.autograd.string_based_ops import StringBasedFunction
        train_set = SATMath(root=kwargs.get("data_dir"), split="train")
        val_set = SATMath(root=kwargs.get("data_dir"), split="val")
        test_set = SATMath(root=kwargs.get("data_dir"), split="test")
        fn_purpose = "The runtime of string-based function that checks if the prediction is correct."
        eval_fn = StringBasedFunction(gpqa_eval_fn, function_purpose=fn_purpose)
        return train_set, val_set, test_set, eval_fn
    
    elif task_name == "math_group":
        from .math_group import MathGroup
        from .mygpqa import gpqa_eval_fn
        from textgrad.autograd.string_based_ops import StringBasedFunction
        train_set = MathGroup(task_name, root=kwargs.get("data_dir"), split="train", *args, **kwargs)
        val_set = MathGroup(task_name, root=kwargs.get("data_dir"), split="val", *args, **kwargs)
        test_set = MathGroup(task_name, root=kwargs.get("data_dir"), split="test", *args, **kwargs)
        fn_purpose = "The runtime of string-based function that checks if the prediction is correct."
        eval_fn = StringBasedFunction(gpqa_eval_fn, function_purpose=fn_purpose)
        return train_set, val_set, test_set, eval_fn
    
    elif task_name == "gaokao_group":
        from .gaokao_group import GaoKaoGroup
        from .mygpqa import gpqa_eval_fn
        from textgrad.autograd.string_based_ops import StringBasedFunction
        train_set = GaoKaoGroup(task_name, root=kwargs.get("data_dir"), split="train", *args, **kwargs)
        val_set = GaoKaoGroup(task_name, root=kwargs.get("data_dir"), split="val", *args, **kwargs)
        test_set = GaoKaoGroup(task_name, root=kwargs.get("data_dir"), split="test", *args, **kwargs)
        fn_purpose = "The runtime of string-based function that checks if the prediction is correct."
        eval_fn = StringBasedFunction(gpqa_eval_fn, function_purpose=fn_purpose)
        return train_set, val_set, test_set, eval_fn
    
    elif task_name == "human_group":
        from .human_group import HumanGroup
        from .mygpqa import gpqa_eval_fn
        from textgrad.autograd.string_based_ops import StringBasedFunction
        train_set = HumanGroup(task_name, root=kwargs.get("data_dir"), split="train", *args, **kwargs)
        val_set = HumanGroup(task_name, root=kwargs.get("data_dir"), split="val", *args, **kwargs)
        test_set = HumanGroup(task_name, root=kwargs.get("data_dir"), split="test", *args, **kwargs)
        fn_purpose = "The runtime of string-based function that checks if the prediction is correct."
        eval_fn = StringBasedFunction(gpqa_eval_fn, function_purpose=fn_purpose)
        return train_set, val_set, test_set, eval_fn
        ...
        from textgrad.autograd.string_based_ops import StringBasedFunction
        train_set = GPQA(root=kwargs.get("data_dir"), split="train")
        val_set = GPQA(root=kwargs.get("data_dir"), split="val")
        test_set = GPQA(root=kwargs.get("data_dir"), split="test")
        fn_purpose = "The runtime of string-based function that checks if the prediction is correct."
        eval_fn = StringBasedFunction(gpqa_eval_fn, function_purpose=fn_purpose)
        return train_set, val_set, test_set, eval_fn
    ...
    elif task_name == "bbeh_boardgame_qa":
        from .bbeh import BigBenchExtraHard, bbeh_freeform_eval_fn
        from textgrad.autograd.string_based_ops import StringBasedFunction
        train_set = BigBenchExtraHard(task_name, split="train", *args, **kwargs)
        val_set = BigBenchExtraHard(task_name, split="val", *args, **kwargs)
        test_set = BigBenchExtraHard(task_name, split="test", *args, **kwargs)
        ...
        from .bbeh import bbeh_mcq_eval_fn
        from textgrad.autograd.string_based_ops import StringBasedFunction
        train_set = LogicalGroup(task_name, split="train", *args, **kwargs)
        val_set = LogicalGroup(task_name, split="val", *args, **kwargs)
        test_set = LogicalGroup(task_name, split="test", *args, **kwargs)
        fn_purpose = "The runtime of string-based function that checks if the prediction is correct."
        eval_fn = StringBasedFunction(bbeh_mcq_eval_fn, function_purpose=fn_purpose)
        return train_set, val_set, test_set, eval_fn
