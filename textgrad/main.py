...
train_set, val_set, test_set, eval_fn = load_task(args.dataset, evaluation_api=llm_api_eval, data_dir=data_dir)
...
# 根据验证集的结果，选择最佳prompt
best_idx = np.argmax([
    np.mean(acc) for acc in results["validation_acc"]
])
best_prompt = results["prompt"][best_idx]
best_test_acc = np.mean(results["test_acc"][best_idx])
print("Best Prompt")
print(best_prompt)
